...
def gridClean(publisher):
    ...
    # Yüzünü yukarı (theta = 90 derece) döndür
    setDesiredOrientation(publisher, 10, 90)  

    # (1,9) noktasına çık
    #go_to_goal(publisher, 1, 9)  
    move(publisher, 1.0, 9.0, True)  # Yukarı git

    for i in range(6):  
        
        rotate(publisher, 10, 90, True)  # 90 derece sağa dön
        move(publisher, 2.0, 1.0, True)  # 1 birim sağa git
        rotate(publisher, 10, 90, True)  # 90 derece sağa dön
        move(publisher, 2.0, 9.0, True)  # Aşağı in
        rotate(publisher, 10, 90, False)  # 90 derece sola dön
        move(publisher, 2.0, 1.0, True)  # 1 birim sağa git
        rotate(publisher, 10, 90, False)  # 90 derece sola dön
...
if __name__ == '__main__':
    try:
        ...
        #go_to_goal(velocity_publisher, 4.0, 2.5)
        #setDesiredOrientation(velocity_publisher, 30, 90)
        #spiralClean(velocity_publisher, 1, 1)
